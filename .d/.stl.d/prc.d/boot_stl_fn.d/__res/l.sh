...
l_02_f2f() {
    ...
    #! THIS FILE WILL BE CREATED: `. ${STL_D_PATH}/prc.d/boot_stl_fn.d/_rbld_l_xx.sh l_02_f2f`
    #! All changes will be lost after rebuilding
    #! To change this file use the following commands:

    #! l_02_f2f _e_prc to edit/change 'fn block',
    #! l_02_f2f _rbld to rebuild fn with changed 'fn block'
    #! l_02_f2f _e_xxx` to edit 'init block'
    ...
    if [[ "_rbld" == "$1" ]]; then
        #! rebuild fn : bcp && ord fn.sh from l_02_f2f.sh , cp fn.prc into fn.sh
        . ${fn_dr}/_rbld_l_xx.sh ${FNN}
        #! up to mem fn
        . ${fn_dr}/$1.sh
        cd "$PPWD" || echo -e "${ECHO_WARN}in fs= file://${fn_nm} , line=${LINENO} , EXEC_FAIL : 'cd $PPWD' : continue${NRM}"
    ...
    #* START fn block from from ${STL_D_PATH}/prc.d/boot_stl_fn.d/__prc/l_02_f2f.prc ------------------
    #[[fn_body]]
#? for copy to help block
...
if [[ -f ${init_file_base}/${result_file_name} ]]; then
    ...
    l_00_echo_ret1 "'$FNN() $*' in file://${fn_nm} , line=${LINENO} :: EXIST_FILE 'file://${init_file_base}/${result_file_name} ' return 1"
    cd "$PPWD" || echo -e "${ECHO_WARN}in fs= file://${fn_nm} , line=${LINENO} , EXEC_FAIL : 'cd $PPWD' : continue${NRM}"
    return 1

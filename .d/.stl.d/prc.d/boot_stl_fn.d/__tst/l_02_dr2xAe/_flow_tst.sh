...
_l_02_dr2xAe_f5e437a_flow() {

    if ! command -v l_02_dr2xAe >/dev/null; then
        l_00_echo_ret1 "TYPE_ERROR : l_02_dr2xAe"
        return 1
    fi

    local filename=${STL_D_PATH}/prc.d/boot_stl_fn.d/__tst/l_02_dr2xAe/_flow_tst.sh
    ...
    l_02_dr2xAe d2 yes &>"${res}"

    l_02_s2f ${STL_D_PATH} @ "${res}"

    # l_02_dr2xAe "${tst_dir}" &>"${res}"

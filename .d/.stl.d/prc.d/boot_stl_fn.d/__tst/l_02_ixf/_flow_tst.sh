...
_l_02_ixf_RND_flow() {
    ...
    if ! command -v l_02_ixf >/dev/null; then
        l_00_echo_ret1 "TYPE_ERROR : l_02_ixf"
        cd $PW
        return 1
    fi

    local filename=${STL_D_PATH}/prc.d/boot_stl_fn.d/__tst/l_02_ixf/_flow_tst.sh
    ...
    #     echo "HERE_STR:"
    #     l_02_ixf <<<"here_str_::_fifth_str" &>>"${res}"

    l_02_ixf ixf.sh "str45" <<EOF &>>"${res}"
    first_str
second_str
EOF

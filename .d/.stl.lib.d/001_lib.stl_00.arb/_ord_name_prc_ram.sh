...
${NORMAL}"
    ...
    cd ${STL_LIB_D_PATH}/lib.${lib_name}.arb

    _sd2d _PRC ${lib_name}_${1} ${STL_LIB_D_PATH}/lib.${lib_name}.arb/_PRC.ram || {
        l_00_echo_ret1 "in fs= file://$path_file , line=${LINENO}, ${FNN}() : FAIL_EXEC '_sd2d _PRC ${lib_name}_${1} ${STL_LIB_D_PATH}/lib.${lib_name}.arb/_PRC.ram' : ${hint} : return 1"
        return 1
    }

    cd ${STL_DATA_D_PATH}/data.${lib_name}.arb

    pwd
    read -p "_sd2d _PRC ${1}_0stl ${STL_DATA_D_PATH}/data.${lib_name}.arb/_PRC.ram"

    _sd2d _PRC ${lib_name}_${1} ${STL_DATA_D_PATH}/data.${lib_name}.arb/_PRC.ram || {
        l_00_echo_ret1 "in fs= file://$path_file , line=${LINENO}, ${FNN}() : FAIL_EXEC '_sd2d _PRC ${lib_name}_${1} ${STL_DATA_D_PATH}/lib.${lib_name}.arb/_PRC.ram' : ${hint} : return 1"
        return 1
    }

...
__stl_00_t4fl_FLOW() {
    ...
    if ! command -v stl_00_t4fl >/dev/null; then
        l_00_echo_ret1 echo "TYPE_ERROR : stl_00_t4fl"
        cd $PW
        return 1
    fi

    local filename=${STL_LIB_D_PATH}/001_lib.stl_00.arb/stl_00_t4fl.ram/.grot/_tst/_flow_tst.sh
